...
import { Plus, Settings, Users, Link as LinkIcon, Send, Sparkles, Filter, ChevronLeft, ChevronRight, Calendar as CalendarIcon, FileText, LayoutGrid, Palette, GripVertical, Inbox } from "lucide-react"
...
export default function ClientWorkflowPage() {
    ...
    const [client, setClient] = useState(MOCK_CLIENTS.find(c => c.id === params.id) || MOCK_CLIENTS[0])
    ...
    useEffect(() => {
        ...
        window.addEventListener("sf-data-change", loadDemands)
        return () => window.removeEventListener("sf-data-change", loadDemands)
    ...
    return (
        <div className="flex h-screen w-full bg-background text-foreground overflow-hidden font-sans">
            ...
            <main className="flex-1 overflow-hidden bg-transparent p-5 relative z-10 flex flex-col gap-3">
                ...
                {/* Header */}
                <div className="rounded-xl border border-white/5 bg-[#0A0A0A] p-4 flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-3">
                        <Avatar className="h-10 w-10 border border-white/10">
                            <AvatarImage src={client.logo} />
                            <AvatarFallback className="bg-gradient-to-br from-orange-500 to-amber-600 text-white font-bold">{client.name[0]}</AvatarFallback>
                        </Avatar>
                        <div>
                    ...
                    <div className="flex gap-1.5">
                        <Button variant="ghost" size="icon-xs" className="text-muted-foreground hover:text-white"><Sparkles className="h-3.5 w-3.5" /></Button>
                        <Button variant="ghost" size="icon-xs" className="text-muted-foreground hover:text-white"><Send className="h-3.5 w-3.5" /></Button>
                        <Button variant="ghost" size="icon-xs" className="text-muted-foreground hover:text-white"><LinkIcon className="h-3.5 w-3.5" /></Button>
                        <Button variant="ghost" size="icon-xs" className="text-muted-foreground hover:text-white"><Settings className="h-3.5 w-3.5" /></Button>
                    </div>
                </div>
                ...
                <div className="flex items-center justify-between gap-3 shrink-0">
                        ...
                        {[
                            ...
                            <Button
                                ...
                                onClick={async () => {
                                    if (item.id === "branding") {
                                        try {
                                            const res = await fetch('/api/clients/open-folder', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ clientName: client.name }) })
                                            const data = await res.json()
                                            if (data.success) toast.success("Pasta Aberta", { description: `Pasta Branding aberta no Finder.` })
                                            else toast.error("Pasta não encontrada", { description: "Crie a pasta primeiro." })
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex bg-[#0A0A0A] rounded-md border border-white/5 p-0.5">
                            <Button
                                size="sm"
                                variant="ghost"
                ...
                <div className="flex gap-3 flex-1 min-h-0">
                    {/* Calendar */}
                    <div className="flex-1 rounded-xl border border-white/5 bg-[#0A0A0A] overflow-hidden flex flex-col min-w-0">
                        {/* Day headers */}
                                        ...
                                        >
                                            ...
                                            {cell.currentMonth && (
                                                <div className="absolute top-0.5 right-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <NewDemandDialog clientId={client.id} clientName={client.name}>
                                                        <Button variant="ghost" size="icon" className="h-5 w-5 text-zinc-600 hover:text-primary hover:bg-primary/10">
                                        ...
                                        >
                                            ...
                                            {cell.currentMonth && (
                                                <div className="mt-1 shrink-0">
                                                    <NewDemandDialog clientId={client.id} clientName={client.name}>
                                                        <Button variant="ghost" size="sm" className="w-full h-6 text-[9px] text-zinc-600 hover:text-primary hover:bg-primary/10 border border-dashed border-transparent hover:border-white/10">
                    ...
                    {/* Backlog Sidebar */}
                    <div
                        className="w-[240px] shrink-0 rounded-xl border border-white/5 bg-[#0A0A0A] flex flex-col overflow-hidden"
                    ...
                    >
                        ...
                        <div className="p-2 border-t border-white/5 shrink-0">
                            <NewDemandDialog clientId={client.id} clientName={client.name} unscheduled>
                                <Button variant="outline" size="sm" className="w-full h-7 text-[10px] border-dashed border-white/10 text-zinc-500 hover:text-white gap-1">
